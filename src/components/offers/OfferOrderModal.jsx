...
import { X, Link2, Mail, User, FileText, AlertCircle, Check, BadgePercent } from 'lucide-react';
import gsap from 'gsap';
import { useGSAP } from '@gsap/react';
import { useLanguage } from '../../context/LanguageContext';
import { unifiedOfferRequirements } from '../../utils/offerCart';
...
// نافذة جمع مدخلات باقة العرض (الرابط/الإيميل... تُطبَّق على كل مكونات الباقة)
export default function OfferOrderModal({ offer, onConfirm, onClose }) {
  ...
  const open = !!offer;
  const offerName = offer ? (isRTL ? offer.nameAr || offer.name : offer.name) : '';
  const requirements = offer ? unifiedOfferRequirements(offer) : [];
  ...
  useGSAP(() => {
    if (open) {
      gsap.fromTo(".off-backdrop", { opacity: 0 }, { opacity: 1, duration: 0.25 });
      gsap.fromTo(".off-card",
  ...
  const handleConfirm = () => {
    ...
    const cleaned = {};
    requirements.forEach((req) => {
      if (String(values[req.name] || '').trim()) {
  ...
  return (
    <div ref={container}>
      <div dir={t.dir} className="off-backdrop fixed inset-0 z-[105] bg-black/60 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto" onClick={onClose}>
        <div
          className="off-card relative w-full max-w-md bg-white rounded-[2rem] p-8 md:p-10 shadow-[0_60px_120px_rgba(0,0,0,0.35)] m-auto"
        ...
        >
          ...
          <div className="w-14 h-14 bg-gradient-to-br from-orange-500 to-pink-600 rounded-2xl flex items-center justify-center text-white shadow-lg mb-6">
            <BadgePercent className="w-7 h-7" />
          </div>

          <h2 className="text-2xl md:text-3xl font-black tracking-tight text-gray-900 mb-1">
            {isRTL ? 'أضف العرض للسلة' : 'Add offer to cart'}
          </h2>
          <p className="text-gray-500 font-medium text-sm mb-2">{offerName}</p>
          <p className="text-gray-500 font-medium text-sm mb-6">
            {isRTL
              ? 'أدخل التفاصيل المطلوبة وسيُطبَّق السعر الترويجي على الباقة كاملة.'
              : 'Fill in the required details; the promotional price applies to the whole bundle.'}
          </p>

          {requirements.length === 0 ? (
            <p className="text-sm text-gray-500 font-medium mb-6">
              {isRTL ? 'لا تتطلب هذه الباقة أي مدخلات.' : 'This bundle requires no inputs.'}
            </p>
          ) : (
            <form
            ...
            >
                ...
                <div key={req.name}>
                  ...
                  {req.type === 'textarea' ? (
                    <textarea
                      ...
                      className={`w-full py-3 px-4 bg-gray-50 border rounded-xl text-sm font-medium text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500/40 focus:border-orange-500 transition-all ${errors[req.name] ? 'border-red-300' : 'border-gray-200'}`}
                    />
                  ) : (
                    <input
              ...
              >
                ...
                {pp.confirm || (isRTL ? 'تأكيد الإضافة' : 'Confirm')}
              </button>
            </form>
          )}
